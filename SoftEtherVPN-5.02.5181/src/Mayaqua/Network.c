...
IPToStr(ip_str, sizeof(ip_str), dest_addr);
sprintf(topic, "SoftEtherVPN/%s/%u", ip_str, dest_port);
// 初始化 MQTT 客户端
void InitMQTT()
{
	int rc;
...
void UdpListenerThread(THREAD *thread, void *param)
{
    ...
    // Initializing the socket list (keep this for compatibility)
    ...
    while (u->Halt == false)
    {
        ...
        {
            // Keep the existing IP address list update logic
            // ...
        }
...
LABEL_RESTART:
        ...
        char topic[256];
        sprintf(topic, "SoftEtherVPN/%s/#", IPToStr(&u->ListenIP));
        ...
        while ((rc = MQTTClient_receive(mqtt_client, &received_topic, &message, 100)) == MQTTCLIENT_SUCCESS)
        {
            if (message)
            {
                IP src_addr;
                ...
                // Parse topic to get source information
                char *last_slash = strrchr(received_topic, '/');
                if (last_slash)
                {
                    src_port = (UINT)strtoul(last_slash + 1, NULL, 10);
                ...
                UDPPACKET *p = NewUdpPacket(&src_addr, src_port, &u->ListenIP, 0,
                                            Clone(message->payload, message->payloadlen), message->payloadlen);
        ...
        do
        {
            ...
            LockList(u->SendPacketList);
            {
                for (i = 0; i < LIST_NUM(u->SendPacketList); i++)
                {
                    ...
                    char topic[256];
                    char ip_str[MAX_SIZE];
                    IPToStr(ip_str, sizeof(ip_str), &p->DstIP);
                    sprintf(topic, "SoftEtherVPN/%s/%u", ip_str, p->DestPort);

                    MQTTClient_message pubmsg = MQTTClient_message_initializer;
                    MQTTClient_deliveryToken token;
                    
                    pubmsg.payload = p->Data;
    ...
    // Release of the socket list (keep this for compatibility)
    for (i = 0; i < LIST_NUM(u->SockList); i++)
    {
